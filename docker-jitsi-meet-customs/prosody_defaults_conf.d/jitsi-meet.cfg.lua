-- Base:
-- https://github.com/jitsi/docker-jitsi-meet/blob/541a53ab881e9aa6dd7804963827e3019e440fac/prosody/rootfs/defaults/conf.d/jitsi-meet.cfg.lua#L124
...
{{ $JWT_ASAP_KEY_SERVER := .Env.JWT_ASAP_KEY_SERVER | default "" -}}
{{ $JWT_ASAP_REQUIRE_ROOM_CLAIM := .Env.JWT_ASAP_REQUIRE_ROOM_CLAIM | default "false" | toBool -}}
{{ $JWT_CACHE_KEYS_URL := .Env.JWT_CACHE_KEYS_URL | default "" -}}
...
{{ if $ENABLE_AUTH }}
  {{ if eq $PROSODY_AUTH_TYPE "jwt" }}
    ...
    {{ if $JWT_ASAP_KEY_SERVER }}
    asap_key_server = "{{ .Env.JWT_ASAP_KEY_SERVER }}"
    {{ end }}
    {{ if $JWT_CACHE_KEYS_URL }}
    cache_keys_url = "{{ .Env.JWT_CACHE_KEYS_URL }}"
    {{ end }}
    asap_require_room_claim = {{ $JWT_ASAP_REQUIRE_ROOM_CLAIM }}
